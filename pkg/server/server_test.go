...
`
	...
	if cfg.Rules[0].Source != "quay.io" || cfg.Rules[0].Target != "my-ecr-quay" {
		t.Errorf("Unexpected rule: %+v", cfg.Rules[0])
	}
	if cfg.Rules[1].Source != "docker.io" || cfg.Rules[1].Target != "my-ecr-docker" {
		t.Errorf("Unexpected rule: %+v", cfg.Rules[1])
	}
}
